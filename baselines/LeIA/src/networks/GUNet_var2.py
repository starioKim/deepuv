...
class GUNet_var(tf.keras.Model):
    def __init__(
        ...
        freq_weights = [np.ones_like(self.measurement_weights)]  # list with weights for each scale
        grad_layers = []
        subsampled_inputs = []
...
#             x = tf.math.real(ops[0].adj_op_sub(y_sub, tf.reshape(sel[0], [-1])))
            x = tf.math.real(ops[0].adj_op_sub(y_sub*self.measurement_weights, tf.reshape(sel[0], [-1])))
