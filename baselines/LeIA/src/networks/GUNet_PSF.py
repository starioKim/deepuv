...
class Gradient(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, psf, shape_x, depth):
        ...
        self.ft_psf = self.ft_psf[
            :, 
            psf.shape[1]//2 - shape_x[0]//2:psf.shape[1]//2 + (shape_x[0]+1)//2, 
            psf.shape[2]//2 - shape_x[1]//2:psf.shape[2]//2 + (shape_x[0]+1)//2
...
class GUNet(tf.keras.Model):
    def __init__(
        ...
        x0 = tf.keras.Input(image_shape, dtype=tf.float32)
        ...
        for i in range(depth+1): 
            ...
            grad = Gradient(
                psf, 
                shape_x=(subsampled_inputs[i].shape[1:]), 
#                 shape_y=(subsampled_inputs[i].shape[1:]), 
        ...
        super().__init__(inputs=[x0, dirty_image, psf], outputs=outputs)
